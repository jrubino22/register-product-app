...
const Index = ({warranties}) => {
    ...
    const warrantyButton = async (_id) => {
        ...
        const response = await fetch(`http://localhost:${process.env.PORT}/warranty/${_id}`, requestOptions);
        await response.json();   
        location.reload()
        return false
    }
...
export async function getServerSideProps(){
    const response = await fetch(`http://localhost:${process.env.PORT}/warranties`)
